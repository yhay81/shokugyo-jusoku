...
if ($KeyFiles.Count -ne 1) { throw "Expected exactly one IndexNow key file" }
...
if ($KeyResponse.StatusCode -ne 200 -or $KeyResponse.Content.Trim() -ne $Key) {
    throw "Published IndexNow key unavailable"
}
$Map = Invoke-WebRequest -Uri "$NormalizedBaseUrl/sitemap.xml" -SkipHttpErrorCheck -TimeoutSec 30
$Urls = @(
    [regex]::Matches($Map.Content, "<loc>([^<]+)</loc>") |
        ForEach-Object { $_.Groups[1].Value }
)
if ($Urls.Count -ne 4) { throw "Expected 4 public URLs" }
$Payload = @{
...
$Response = Invoke-WebRequest -Uri "https://api.indexnow.org/indexnow" -Method Post `
    -ContentType "application/json; charset=utf-8" -Body $Payload -SkipHttpErrorCheck -TimeoutSec 60
if ($Response.StatusCode -notin @(200, 202)) { throw "IndexNow submission failed" }
[ordered]@{
